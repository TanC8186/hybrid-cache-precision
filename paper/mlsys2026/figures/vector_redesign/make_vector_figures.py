...
def fig2_gsm8k() -> None:
    ...
    for yi, (label, row) in zip(y, entries):
        ...
        draw_ci(ax, yi, est, ci, color=color, size=28)
    ...
    ax.text(3.25, 5.55, "200 items x 9 paired dataset seeds", color=GRAY_DARK,
...
def fig4_serving() -> None:
    ...
    outer = fig.add_gridspec(1, 2, width_ratios=[1.65, 1.0], left=0.055,
                             right=0.995, top=0.84, bottom=0.20, wspace=0.34)
    ...
    for i, rate in enumerate(rates):
        ...
        if i == 0:
            ax.set_ylabel("Paired goodput change\n(bf16 - fp32 state)")
        else:
    ...
    axes[0].legend(
        handles=[
            ...
            Line2D([], [], marker="D", color=BLUE, lw=0, label="second run", ms=4),
        ],
        loc="upper left",
    ...
    for display, key in workloads:
        for run_name, js in (("formal", formal), ("second", second)):
            vals = []
            for t in thresholds:
...
def fig8_gsm8k_per_seed() -> None:
    ...
    def draw(ax: mpl.axes.Axes, rows: list[dict], allocs: list[str], labels: list[str],
        ...
        means = np.array([by[a]["mean"] for a in allocs]) * 100.0
    ...
    draw(
        ...
        ["fp16 KV\nfp32 state", "fp16 KV\nbf16 state",
         "int4 KV\nfp32 state", "int4 KV\nbf16 state"],
        "2B - paired trajectories across 9 dataset seeds",
        2.1,
    )
    draw(
        axes[1],
        ...
        ["fp16", "fp16_statebf16"],
        ["fp16 KV\nfp32 state", "fp16 KV\nbf16 state"],
        "9B - paired trajectories across 9 dataset seeds",
        1.2,
    )
    axes[0].text(0.99, 0.04, "gray: individual seeds   blue: seed mean",
