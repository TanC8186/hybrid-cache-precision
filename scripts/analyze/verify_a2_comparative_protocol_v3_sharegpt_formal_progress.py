"""Audit cumulative progress for the sliced A2 protocol-v3 ShareGPT formal run."""
...
PARENT_ID = (
    "a2-comparative-serving-random60-formal-v3-piecewise-3108650-westd-01"
)
CONFIG_NAME = "a2_comparative_piecewise_protocol_v3_sharegpt300_formal.yaml"
CONFIG_SHA256 = "13888902c80bda29ce282969ac3200789006554f4373fd1340ee48d8b991aca8"
PHASE_NAME = "comparative_sharegpt300_formal_v3"
PLAN_SIZE = 63
SLICE_SIZE = 5
MAX_SLICES = (PLAN_SIZE + SLICE_SIZE - 1) // SLICE_SIZE
ALLOCATIONS = ("fp16", "int4", "packed_per_layer")


def completed_count_for_slice(slice_number: int) -> int:
    require(1 <= slice_number <= MAX_SLICES, "slice number outside formal plan")
    return min(slice_number * SLICE_SIZE, PLAN_SIZE)


def expected_server_allocations(
    plan: Sequence[Mapping[str, Any]],
    slice_number: int,
) -> tuple[str, ...]:
    completed_count = completed_count_for_slice(slice_number)
    require(len(plan) == PLAN_SIZE, "formal plan sample count")
    expected: list[str] = []
    for start in range(0, completed_count, SLICE_SIZE):
        previous: str | None = None
        for item in plan[start : min(start + SLICE_SIZE, completed_count)]:
            allocation = str(item["allocation"])
            require(allocation in ALLOCATIONS, "formal plan allocation")
...
def expected_sidecar_count(
    completed_count: int,
    server_allocations: Sequence[str],
) -> int:
    return 3 + 3 * completed_count + len(server_allocations)


def classify_sharegpt_formal_progress(
    *,
    slice_number: int,
    ...
    if all(
        ...
        return f"PROTOCOL_V3_SHAREGPT_FORMAL_SLICE_{suffix}_PASSED"
    return f"PROTOCOL_V3_SHAREGPT_FORMAL_SLICE_{suffix}_REVIEW_REQUIRED"
...
def audit_progress(raw_dir: Path, slice_number: int) -> dict[str, Any]:
    ...
    expected_allocations = expected_server_allocations(
        contract["plan"],
        slice_number,
    )
    ...
    expected_sidecars = expected_sidecar_count(
        completed_count,
    ...
    return {
        ...
        "samples": samples,
        "next_gate": (
            f"Continue the same ShareGPT formal attempt with "
            f"slice-{slice_number + 1:03d} and --resume."
            if completed_count < PLAN_SIZE
            else "Run the independent reproducibility attempt."
        ),
    }
...
- Version Label: a2_protocol_v3_sharegpt_formal_progress_v1
...
claims remain blocked until independent reproducibility passes.
"""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    stem = f"protocol_v3_sharegpt_formal_{report['slice_id'].replace('-', '_')}"
