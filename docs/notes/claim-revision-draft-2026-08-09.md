...
- GSM8K：**真实随机协议（每 seed 独立抽 200 题）下 2B state 点估计 −1.33pt、
  9B 0.00pt，3-seed 配对 CI 均含 0**；旧 head-200 协议的 −2.67pt/−0.5pt 是
  固定题目集伪重复产物，不再作为显著性证据。

红线：GSM8K 不得再写“CI 不含 0 的真实回退”；如需更强结论，补 seed 数并做
MDE/等价性分析（S3）。
